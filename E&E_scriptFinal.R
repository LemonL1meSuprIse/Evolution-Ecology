...
#import df
all_interest_trimmed<-read.csv("/jittered_all_ interest.csv")
...
CollapsedLanduse<-raster("/Rasters/Land_use/CollapsedLanduse.tif")

#create raster stack
stack<-stack(Aspect_North,Slope_Deg,Temp_Max,Temp_Min,sol_Ann,precip_Ann,CollapsedLanduse)

rm(Aspect_North,Slope_Deg,Temp_Max,Temp_Min,Geomorphons,sol_Ann,precip_Ann,CollapsedLanduse)
...
GRA_mod<- ENMevaluate(occs = GRA_chck, envs = GRA_bg, bg=GRA_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

MACplus_mod<- ENMevaluate(occs = MACplus_chck, envs = MACplus_bg, bg=MACplus_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L","Q","LQ"), rm = 1:5))

MCC_mod<- ENMevaluate(occs = MCC_chck, envs = MCC_bg, bg=MCC_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

RMM_mod<- ENMevaluate(occs = RMM_chck, envs = RMM_bg, bg=RMM_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

SAG_mod<- ENMevaluate(occs = SAG_chck, envs = SAG_bg, bg=SAG_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

SCR_mod<- ENMevaluate(occs = SCR_chck, envs = SCR_bg, bg=SCR_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

RMMlong_mod<- ENMevaluate(occs = RMMlong_chck, envs = RMMlong_bg, bg=RMMlong_bg_points, algorithm = 'maxnet', partitions = 'checkerboard1',categoricals = c("CollapsedLanduse"),tune.args = list(fc=c("L", "Q","LQ"), rm = 1:5))

##-----------Dropping categorical levels to test over fitting issues-------------
GRA_bg_DROPPED <- dropLayer(GRA_bg,c("topo_geomorphons"))
...
opt.seqGRA
tables<-as.data.frame(opt.seqGRA)
tables$species<-"GRA"
...
# optimal model.
mod.seqGRA <- eval.models(GRA_mod)[[opt.seqGRA$tune.args]]
...
# (Phillips et al. 2017).
opt.seqGRA
m1.mxGRA <- eval.models(GRA_mod)[["fc.LQ_rm.5"]]
plot(m1.mxGRA, type="cloglog")
# model object above.
predGRA <- eval.predictions(GRA_mod)[[opt.seqGRA$tune.args]]
plot(predGRA)
#writeRaster(predGRA, "E:/Chapter_3 Modelling/QGIS/predGRA.tif",overwrite=TRUE)
...
# Overall results
resMAC <- eval.results(MACplus_mod)
...
# We can select a single model from the ENMevaluation object using the tune.args of our
# optimal model.
mod.seqMAC <- eval.models(MACplus_mod)[[opt.seqMAC$tune.args]]
...
# (Phillips et al. 2017).
opt.seqMAC
m1.mxMAC <- eval.models(MACplus_mod)[["fc.L_rm.1"]]
plot(m1.mxMAC, type="cloglog")
# model object above.
predMACplus <- eval.predictions(MACplus_mod)[[opt.seqMAC$tune.args]]

#writeRaster(predMACplus, "E:/Chapter_3 Modelling/QGIS/predMACplus.tif",overwrite=TRUE)
...
# We can select a single model from the ENMevaluation object using the tune.args of our
# optimal model.
mod.seqMCC <- eval.models(MCC_mod)[[opt.seqMCC$tune.args]]
...
# (Phillips et al. 2017).
opt.seqMCC
m1.mxMCC <- eval.models(MCC_mod)[["fc.Q_rm.1"]]
plot(m1.mxMCC, type="cloglog")
# model object above.
predMCC <- eval.predictions(MCC_mod)[[opt.seqMCC$tune.args]]

#writeRaster(predMCC, "E:/Chapter_3 Modelling/QGIS/predMCC.tif",overwrite=TRUE)
...
# We can select a single model from the ENMevaluation object using the tune.args of our
# optimal model.
mod.seqSAG <- eval.models(SAG_mod)[[opt.seqSAG$tune.args]]
...
# (Phillips et al. 2017).
opt.seqSAG
SAGm1.mx <- eval.models(SAG_mod)[["fc.Q_rm.5"]]
plot(SAGm1.mx,type="cloglog")
# model object above.
predSAG <- eval.predictions(SAG_mod)[[opt.seqSAG$tune.args]]


#writeRaster(predSAG, "E:/Chapter_3 Modelling/QGIS/predSAG.tif",overwrite=TRUE)
...
# We can select a single model from the ENMevaluation object using the tune.args of our
# optimal model.
mod.seqSCR <- eval.models(SCR_mod)[[opt.seqSCR$tune.args]]
...
# (Phillips et al. 2017).
opt.seqSCR
SCRm1.mx <- eval.models(SCR_mod)[["fc.Q_rm.2"]]

plot(SCRm1.mx,type="cloglog")
# model object above.
predSCR <- eval.predictions(SCR_mod)[[opt.seqSCR$tune.args]]

#writeRaster(predSCR, "E:/Chapter_3 Modelling/QGIS/predSCR.tif",overwrite=TRUE)
...
m1.mx <- eval.models(RMMlong_mod)[["fc.L_rm.2"]]
m1.mx$betas

# model object above.
predRMMlong <- eval.predictions(RMMlong_mod)[[opt.seq$tune.args]]

#writeRaster(predRMMlong, "E:/Chapter_3 Modelling/QGIS/predRMMlong.tif",overwrite=TRUE)
...
presenceRMMlong<-subset(PresenceTEST,PresenceTEST$RMM==1)
absenceRMMlong<-subset(PresenceTEST,PresenceTEST$RMM==0)
...
###------------Null models------------
###------GRA-------
mod.nullGRA <- ENMnulls(GRA_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullGRA) %>% head()

null.results.partitions(mod.nullGRA) %>% head()

null.emp.results(mod.nullGRA)

evalplot.nulls(mod.nullGRA, stats = c("auc.val"), plot.type = "histogram")

GRAnullplot<-evalplot.nulls(mod.nullGRA, stats = c("auc.val"), plot.type = "histogram")

GRAplot<-evalplot.nulls(mod.nullGRA, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
GRAplot<-as.data.frame(GRAplot)

ggplot(data = GRAplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF", binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = GRAplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  coord_cartesian(xlim = c(0.25,1.00))

###------MAC-------
mod.nullMACplus <- ENMnulls(MACplus_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullMACplus) %>% head()

null.results.partitions(mod.nullMACplus) %>% head()

null.emp.results(mod.nullMACplus)

evalplot.nulls(mod.nullMACplus, stats = c("auc.val"), plot.type = "histogram")

MACplusnullplot<-evalplot.nulls(mod.nullMACplus, stats = c("auc.val"), plot.type = "histogram")

MACplusplot<-evalplot.nulls(mod.nullMACplus, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
MACplusplot<-as.data.frame(MACplusplot)

ggplot(data = MACplusplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF", binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = MACplusplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  coord_cartesian(xlim = c(0.25,1.00))
...
###------MCC-------
mod.nullMCC <- ENMnulls(MCC_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullMCC) %>% head()

null.results.partitions(mod.nullMCC) %>% head()

null.emp.results(mod.nullMCC)

evalplot.nulls(mod.nullMCC, stats = c("auc.val"), plot.type = "histogram")

MCCnullplot<-evalplot.nulls(mod.nullMCC, stats = c("auc.val"), plot.type = "histogram")

MCCplot<-evalplot.nulls(mod.nullMCC, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
MCCplot<-as.data.frame(MCCplot)

ggplot(data = MCCplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF", binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = MCCplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  coord_cartesian(xlim = c(0.25,1.00))

###------SAG-------
mod.nullSAG <- ENMnulls(SAG_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullSAG) %>% head()

null.results.partitions(mod.nullSAG) %>% head()

null.emp.results(mod.nullSAG)

evalplot.nulls(mod.nullSAG, stats = c("auc.val"), plot.type = "histogram")

SAGnullplot<-evalplot.nulls(mod.nullSAG, stats = c("auc.val"), plot.type = "histogram")

SAGplot<-evalplot.nulls(mod.nullSAG, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
SAGplot<-as.data.frame(SAGplot)

ggplot(data = SAGplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF", binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = SAGplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  coord_cartesian(xlim = c(0.25,1.00))
...
###------SCR-------
mod.nullSCR <- ENMnulls(SCR_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullSCR) %>% head()

null.results.partitions(mod.nullSCR) %>% head()

null.emp.results(mod.nullSCR)

evalplot.nulls(mod.nullSCR, stats = c("auc.val"), plot.type = "histogram")

SCRnullplot<-evalplot.nulls(mod.nullSCR, stats = c("auc.val"), plot.type = "histogram")

SCRplot<-evalplot.nulls(mod.nullSCR, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
SCRplot<-as.data.frame(SCRplot)

ggplot(data = SCRplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF", binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = SCRplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  coord_cartesian(xlim = c(0.25,1.00))
...
###------RMMlong-------
mod.nullRMMlong <- ENMnulls(RMMlong_mod, mod.settings = list(fc = "LQ", rm = 4), no.iter = 100)

null.results(mod.nullRMMlong) %>% head()

null.results.partitions(mod.nullRMMlong) %>% head()

null.emp.results(mod.nullRMMlong)

rmmlongplot<-evalplot.nulls(mod.nullRMMlong, stats = c("auc.val"), plot.type = "histogram",return.tbl = TRUE)
rmmlongplot<-as.data.frame(rmmlongplot)

ggplot(data = rmmlongplot, aes(x=null.avgs.avg)) +
  geom_histogram(colour="black",fill="#FDE725FF",binwidth = 0.025) +
  theme_classic() +
  coord_cartesian(xlim=c(0,1)) +
  ylab("") +
  xlab("") +
  geom_vline(xintercept = rmmlongplot$empirical.results.avg, colour="#39568CFF",size=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  coord_cartesian(xlim = c(0.25,1.00))
